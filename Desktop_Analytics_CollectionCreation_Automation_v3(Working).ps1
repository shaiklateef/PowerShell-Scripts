...
Foreach ($Department in $Departments){
    ...
    if((Get-CMCollection -Name "$DepartmentTrimmed" -CollectionType User) -eq $null)
    {
        ...
        $NewCollection = New-CMCollection -LimitingCollectionId FRW0002D -Name "$DepartmentTrimmed" -RefreshSchedule $Schedule1 -RefreshType Periodic -CollectionType User
    ...
    Write-Host $Users
    Write-Host "Fetching primary devices for Users from $DepartmentTrimmed user collection"
