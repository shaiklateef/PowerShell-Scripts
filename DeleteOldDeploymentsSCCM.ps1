	...
	If ((Read-Host -Prompt "Type `"Y`" to delete the deployment, any other key to skip") -ieq "Y")
	{
		Try
		{
			...
			Switch ($Deployment.FeatureType)
			{
				#Application
				1{
					Remove-CMDeployment -DeploymentId $DeploymentID -ApplicationName $DeploymentName -ErrorAction Stop -Force
...
<#	
	...
	===========================================================================
	 ...
	 Created on:   	10/19/2021 10:56 AM
	 Created by:   	adm_slateef
	 Organization: 	
	 Filename:     	
	===========================================================================
	.DESCRIPTION
